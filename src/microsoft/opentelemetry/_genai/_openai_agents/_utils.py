...
from microsoft.opentelemetry.a365.core.utils import safe_json_dumps
...
def get_span_kind(obj: SpanData) -> str:  # pylint: disable=too-many-return-statements
...
def get_attributes_from_response_custom_tool_call_output_param(
    ...
    if (output := obj.get("output")) is not None:
        yield f"{prefix}{GEN_AI_TOOL_CALL_RESULT_KEY}", str(output)
...
def get_attributes_from_function_call_output(
    ...
    yield f"{prefix}{GEN_AI_TOOL_CALL_RESULT_KEY}", obj["output"]  # type: ignore[misc]
                        ...
                        if isinstance(tc, dict):
                            ...
                            if call_id and func_name:
                                ...
                                while len(pending_tool_calls) > max_size:
                                    pending_tool_calls.popitem(last=False)  # type: ignore[call-arg]
    except Exception:  # pylint: disable=broad-exception-caught
        pass
...
def capture_input_message(parent_span_id: str, input_list: Any, agent_inputs: dict[str, str]) -> None:
...
def capture_output_message(parent_span_id: str, output_list: Any, agent_outputs: dict[str, str]) -> None:
